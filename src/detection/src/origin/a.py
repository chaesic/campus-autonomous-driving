...
def main():
    ...
    print(f"[INFO] Detection model loaded from {DET_MODEL}")
    print(f"[INFO] Pose model loaded from {POSE_MODEL}")
    ...
    while True:
        ...
        if crops:
            pose_results_batch = pose.predict(crops, imgsz=IMG_SIZE_POSE, conf=POSE_CONF_THR, verbose=False, device=DEVICE)
